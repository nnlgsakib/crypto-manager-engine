...
export class IndexerService {
  ...
  private initializeProviders() {
    for (const [network, netConfig] of Object.entries(networks)) {
      ...
      for (const [tokenName, token] of Object.entries(netConfig.erc20Tokens)) {
        const provider =
          this.wsProviders[network] || this.httpProviders[network];
      ...
      this.pendingQueues.set(
        `${network}:${netConfig.nativeCurrency}`,
        new Set()
      );
    }
  }
  ...
  private setupWebSocketProvider(network: string, wsUrl: string) {
    try {
      ...
      ws.on('open', () => {
        ...
        for (const [tokenName, contract] of Object.entries(
          this.erc20Contracts[network]
        )) {
      ...
      ws.on('close', () => {
        if (this.reconnectionAttempts[network] >= maxReconnectAttempts) {
          logger.error(
            `Max reconnection attempts reached for ${network}. Using HTTP polling for indexing fallback.`
          );
        ...
        const delay =
          this.WS_RECONNECT_DELAY_BASE *
          Math.pow(2, this.reconnectionAttempts[network] - 1);
      ...
      logger.error(
        `Failed to initialize WebSocket for ${network}: ${err.message}. Using HTTP polling for indexing fallback.`
      );
      this.startHttpPolling(network);
    }
  }
  ...
  private async verifyNetwork(network: string) {
    try {
      const chainId = await this.wsProviders[network]
        .getNetwork()
        .then(n => n.chainId);
      if (chainId !== networks[network].chainId) {
        logger.error(
          `Chain ID mismatch for ${network}: expected ${networks[network].chainId}, got ${chainId}`
        );
      } else {
  ...
  private startHttpPolling(network: string) {
    ...
    setInterval(async () => {
      try {
        if (
          this.wsProviders[network] &&
          this.wsProviders[network]._websocket.readyState === _ws.OPEN
        ) {
          return;
        }
        ...
        if (currentBlock > lastBlockNumber) {
          for (
            ...
            logger.debug(
              `Processed block ${blockNumber} via HTTP polling for ${network}`
            );
          }
  ...
  private async setupBlockListener(network: string) {
    ...
    if (!provider) {
      logger.warn(
        `No WebSocket provider for ${network}. Relying on HTTP polling for indexing.`
      );
      return;
    }
    provider.on('block', async (blockNumber: number) => {
      try {
        logger.debug(
          `Received block ${blockNumber} for ${network} via WebSocket`
        );
        await this.processBlock(network, blockNumber, true);
      } catch (err: any) {
        logger.error(
          `Error processing block ${blockNumber} on ${network}: ${err.message}`
        );
      }
    });
  }
  ...
  private async processBlock(
    ...
    const provider =
      useWebSocket && this.wsProviders[network]
        ? this.wsProviders[network]
        : this.httpProviders[network];
    try {
      ...
      logger.debug(
        `Cached and processed block ${blockNumber} for ${network} using ${useWebSocket ? 'WebSocket' : 'HTTP'}`
      );
    } catch (err: any) {
      logger.error(
        `Failed to process block ${blockNumber} for ${network}: ${err.message}`
      );
    }
  }

  private setupErc20Listeners(
    network: string,
    tokenName: string,
    ...
    contract.on(
      ...
      async (
        ...
        try {
          ...
          logger.debug(
            `Detected Transfer event for ${tokenName} on ${network}: ${txHash} to ${toAddress}`
          );
          if (
            ...
            if (account && networks[network].erc20Tokens[tokenName]) {
              ...
              if (tx) {
                ...
                logger.info(
                  `Queued ERC-20 deposit ${txHash} for ${account.username} (${tokenName}) on ${network}`
                );
              } else {
                logger.warn(
                  `Transaction ${txHash} not found for ${tokenName} deposit to ${toAddress}`
                );
              }
            } else {
              logger.debug(
                `No account or token config for ${toAddress} or ${tokenName} on ${network}`
              );
            }
          }
        } catch (err: any) {
          logger.error(
            `Error processing ERC-20 Transfer ${event.transactionHash || 'unknown'} on ${network}:${tokenName}: ${err.message}`
          );
        }
      }
    );
  ...
  async initialize() {
    ...
    accounts.forEach(a => {
      ...
      logger.debug(
        `Added account address ${a.value.address} for user ${a.value.username}`
      );
    });
    logger.info(
      `Loaded ${accounts.length} active addresses: ${Array.from(this.activeAddresses).join(', ')}`
    );
    ...
    setInterval(async () => {
      try {
        ...
        const lastProcessedBlock = parseInt(
          (await db.get(`lastProcessedBlock:${network}`)) || '0',
          10
        );
        const startBlock = Math.max(
          lastProcessedBlock - this.RECOVERY_LOOKBACK_BLOCKS,
          0
        );
        if (currentBlock <= lastProcessedBlock) {
          logger.debug(
            `No new blocks to recover for ${network}: current=${currentBlock}, lastProcessed=${lastProcessedBlock}`
          );
          return;
        }

        logger.info(
          `Recovering blocks ${startBlock} to ${currentBlock} for ${network}`
        );
        ...
        logger.error(
          `Error during block recovery for ${network}: ${err.message}`
        );
      }
      ...
      try {
        ...
        for (const blockCache of blocks) {
          if (
            ...
            currentBlock - networkConfig.requiredConfirmations
          ) {
            continue;
          }
          ...
          for (const tx of block.transactions) {
            if (
              ...
              if (account && nativeCurrency) {
                logger.debug(
                  `Detected native deposit ${tx.hash} to ${tx.to} on ${network}`
                );
                promises.push(
          ...
          logger.debug(
            `Processed and deleted block cache ${blockCache.blockNumber} for ${network}`
          );
        }
      } catch (err: any) {
  ...
  private async startBlockCleanup(network: string) {
    setInterval(async () => {
      try {
        ...
        logger.error(
          `Error cleaning up expired blocks for ${network}: ${err.message}`
        );
      }
  ...
  private async queueTransaction(
    ...
    const decimals = isNative
      ? 18
      : networkConfig.erc20Tokens[currencyName].decimals;
    ...
    if (amount < minDeposit) {
      logger.info(
        `Tx ${tx.hash} amount ${amount} ${currencyName} below min_deposit ${minDeposit}`
      );
      return;
    }
  ...
  private async processDeposit(
    ...
    try {
      ...
      const startBlock = parseInt(
        await db.get(`depositStartBlock:${txHash}`),
        10
      );
      const confirmations = currentBlock - startBlock + 1;

      if (
        confirmations >= networkConfig.requiredConfirmations &&
        ...
        const transferTxHash = await TransactionService.transferToHotWallet(
          deposit,
          provider
        );
      ...
      if (
        ...
        err.message.includes('Insufficient funds after gas')
      ) {
        deposit.status = 'failed';
        deposit.retries = deposit.retries || 0;
        await DepositModel.update(deposit);
        logger.error(
          `Failed deposit ${txHash} on ${network}:${currency} permanently: ${err.message}`
        );
        WebSocketService.broadcast({
          type: 'deposit_update',
          data: deposit,
      ...
      if (retries > this.MAX_RETRIES) {
        ...
        await DepositModel.update(deposit);
        logger.error(
          `Failed deposit ${txHash} on ${network}:${currency} after ${retries} retries: ${err.message}`
        );
        WebSocketService.broadcast({
          type: 'deposit_update',
          data: deposit,
      ...
      await DepositModel.update(deposit);
      logger.warn(
        `Retry ${retries}/${this.MAX_RETRIES} for deposit ${txHash}: ${err.message}`
      );
