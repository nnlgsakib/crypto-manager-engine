...
export class BatchProcessorService {
  ...
  static async addToBucket(withdrawal: Withdrawal, provider: ethers.providers.JsonRpcProvider): Promise<Bucket> {
    try {
      ...
      logger.error(`Error adding withdrawal ${withdrawal.id} to bucket: ${err.message}`);
      throw err;
    }
  }

  private static scheduleBucketProcessing(bucket: Bucket, provider: ethers.providers.JsonRpcProvider) {
  ...
  static async processBucket(bucketId: string, provider: ethers.providers.JsonRpcProvider) {
    ...
    try {
      ...
      const validWithdrawals = withdrawals.filter((w) => w && w.status === "added_to_bucket") as Withdrawal[];
      ...
      for (const w of validWithdrawals) {
        ...
        amounts.push(
          ethers.utils.parseUnits(
            w.amount,
            w.currency === networkConfig.nativeCurrency ? 18 : networkConfig.erc20Tokens[w.currency].decimals
          )
        );
      }
      ...
      try {
        if (isNative) {
          const totalAmount = amounts.reduce((sum, a) => sum.add(a), ethers.BigNumber.from(0));
          const hotWalletBalance = await provider.getBalance(wallet.address);
          if (hotWalletBalance.lt(totalAmount)) {
            logger.error(`Hot wallet lacks sufficient ${bucket.currency} balance (available: ${ethers.utils.formatEther(hotWalletBalance)}, required: ${ethers.utils.formatEther(totalAmount)})`);
            throw new Error(`Insufficient ${bucket.currency} liquidity in hot wallet for bucket ${bucketId}`);
          }

          let estimatedGas;
          try {
            estimatedGas = await contract.estimateGas.processBatchNative(recipients, amounts, { value: totalAmount });
          } catch (err: any) {
            logger.warn(`Failed to estimate gas for processBatchNative, using default: ${err.message}`);
          ...
          const totalAmount = amounts.reduce((sum, a) => sum.add(a), ethers.BigNumber.from(0));
          const hotWalletTokenBalance = await tokenContract.balanceOf(wallet.address);
          if (hotWalletTokenBalance.lt(totalAmount)) {
            logger.error(`Hot wallet lacks sufficient ${bucket.currency} balance (available: ${ethers.utils.formatUnits(hotWalletTokenBalance, tokenConfig.decimals)}, required: ${ethers.utils.formatUnits(totalAmount, tokenConfig.decimals)})`);
            throw new Error(`Insufficient ${bucket.currency} liquidity in hot wallet for bucket ${bucketId}`);
          }

          const currentAllowance = await tokenContract.allowance(wallet.address, networkConfig.withdrawal_processor_contract_address);
          if (currentAllowance.lt(totalAmount)) {
            ...
            try {
              ...
              logger.warn(`Failed to estimate gas for approval, using default: ${err.message}`);
              approveGas = ethers.BigNumber.from(this.DEFAULT_APPROVE_GAS_LIMIT);
            }
            ...
            logger.info(`Approved ${ethers.utils.formatUnits(totalAmount, tokenConfig.decimals)} ${bucket.currency} for BatchProcessor`);
          }

          let estimatedGas;
          try {
            estimatedGas = await contract.estimateGas.processBatchErc20(tokenConfig.address, recipients, amounts);
          } catch (err: any) {
            logger.warn(`Failed to estimate gas for processBatchErc20, using default: ${err.message}`);
            estimatedGas = ethers.BigNumber.from(this.DEFAULT_ERC20_GAS_LIMIT);
          }
          tx = await contract.processBatchErc20(tokenConfig.address, recipients, amounts, {
            gasPrice,
            gasLimit: estimatedGas.mul(120).div(100),
          });
        }
        ...
        for (const w of validWithdrawals) {
          ...
          if (receipt.status === 1) {
            w.status = "completed";
            w.txHash = tx.hash;
            const balance = await BalanceModel.getBalance(w.username, w.blockchain, w.currency);
            const newBalance: Balance = {
              username: w.username,
              blockchain: w.blockchain,
              currency: w.currency,
              amount: parseFloat(balance.amount).toFixed(2),
              frozenAmount: (parseFloat(balance.frozenAmount || "0") - totalReserved).toFixed(2),
            };
            await Promise.all([
        ...
        logger.error(`Error executing batch transaction for bucket ${bucketId}: ${err.message}`);
        const fee = parseFloat(networkConfig.withdrawal_fee || "0");
        for (const w of validWithdrawals) {
        ...
        if (newBucketId !== bucketId) {
          ...
          logger.info(`Created new bucket ${newBucketId} for ${bucket.currency} on ${bucket.blockchain}`);
        }
      }
    } catch (err: any) {
