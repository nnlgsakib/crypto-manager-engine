...
import { BalanceModel, WithdrawalModel, AccountModel, Balance, Withdrawal } from "../../db/models";
...
router.post("/send_balance_to_user", async (req, res) => {
  try {
    const { username, recipientUsername, blockchain, currency, amount } = req.body;
    ...
    const balance = await BalanceModel.getBalance(username, blockchain, currency);
    ...
    const recipientBalance = await BalanceModel.getBalance(recipientUsername, blockchain, currency);
    const newRecipientBalance: Balance = {
      username: recipientUsername,
      blockchain,
      currency,
      amount: (parseFloat(recipientBalance.amount || "0") + parsedAmount).toFixed(2),
...
router.post("/req_deposit", async (req, res) => {
  try {
    ...
    const config = isNative ? networkConfig : networkConfig.erc20Tokens[currency];
    if (!config) {
    ...
    if (parsedAmount < minWithdrawal) {
      return res.status(400).json({ error: `Withdrawal amount below minimum (${minWithdrawal} ${currency})` });
    }
    if (parsedAmount > maxWithdrawal) {
      return res.status(400).json({ error: `Withdrawal amount above maximum (${maxWithdrawal} ${currency})` });
    }
    ...
    await BalanceModel.freezeBalance(username, blockchain, currency, required.toFixed(2));
    ...
    const provider = new ethers.providers.JsonRpcProvider(networkConfig.httpRpcUrl);
    const bucket = await BatchProcessorService.addToBucket(withdrawal, provider);
    res.json({
